...
export function ChatWidget() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(EV_OPEN_AI_CHAT, onOpen);
    return () => window.removeEventListener(EV_OPEN_AI_CHAT, onOpen);
  }, []);
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 429 || data.code === "RATE_LIMIT") {
        setMessages((m) => [
          ...m,
          {
            ...
            content:
              data.error ||
              `Easy there—${AI_BRAND.name} needs a breath. Try again in a minute.`,
            variant: "rate-limit",
          },
        ]);
        return;
      ...
      const reply =
        data.reply ||
        data.error ||
        `${AI_BRAND.name} could not answer just now—try again?`;
      setMessages((m) => [
        ...m,
      ...
      setMessages((m) => [
        ...
        {
          role: "assistant",
          content: "Connection slipped—check your network and tap send again.",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <button
        ...
        className="ev-touch fixed z-[80] flex min-h-[56px] max-w-[min(calc(100vw-1.5rem),20rem)] items-center gap-3 rounded-full border border-[var(--ev-border-strong)] bg-[var(--ev-surface)] py-3 pl-4 pr-5 text-left transition-transform active:scale-[0.98] motion-safe:animate-[ev-chat-fab-glow_4s_ease-in-out_infinite] max-[380px]:right-3 max-[380px]:min-h-[52px] max-[380px]:pl-3 max-[380px]:pr-4"
        style={{
          bottom: "max(1.25rem, env(safe-area-inset-bottom, 0px))",
          right: "max(1.25rem, env(safe-area-inset-right, 0px))",
        }}
        aria-label={AI_BRAND.chatAriaLabel}
      >
        <span
          className="relative flex size-10 shrink-0 items-center justify-center rounded-full bg-[var(--ev-primary)]/15 ring-1 ring-[var(--ev-border-strong)]"
          aria-hidden
        >
          <span className="font-display text-sm text-[var(--ev-primary)]">I</span>
          <span className="pointer-events-none absolute inset-0 rounded-full border border-[var(--ev-primary)]/35 motion-safe:animate-[ev-chat-ring_2.8s_ease-out_infinite] motion-reduce:animate-none" />
        </span>
        <span className="min-w-0 flex-1">
          <span className="block text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--ev-primary)]">
            {AI_BRAND.chatButtonTeaser}
          </span>
          <span className="mt-0.5 block truncate text-xs font-medium text-[var(--ev-text)]">
            {AI_BRAND.chatButtonLabel}
          </span>
        </span>
      </button>
      {open && (
              ...
              <div className="flex items-center justify-between border-b border-[var(--ev-border)] px-4 py-4 sm:py-3">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.35em] text-[var(--ev-primary)]">
                    {AI_BRAND.panelEyebrow}
                  </p>
                  <p className="font-display text-xl sm:text-lg">{AI_BRAND.panelTitle}</p>
                  <p className="mt-1 max-w-[14rem] text-[10px] leading-snug text-[var(--ev-text-muted)]">
                    {AI_BRAND.tagline}
                  </p>
                </div>
                <button
                  type="button"
                  className="ev-touch min-h-12 min-w-12 shrink-0 rounded-lg text-xs uppercase tracking-[0.2em] text-[var(--ev-text-muted)] transition-colors hover:bg-white/5"
                  onClick={close}
                >
                  Close
                </button>
                    ...
                    <div
                      className={`max-w-[min(92%,20rem)] rounded-[var(--ev-radius-sm)] px-3.5 py-3 text-[15px] leading-relaxed sm:max-w-[92%] sm:text-sm ${
                        m.role === "user"
                          ...
                          : m.variant === "rate-limit"
                            ? "border border-amber-500/35 bg-amber-950/25 text-[var(--ev-text)]"
